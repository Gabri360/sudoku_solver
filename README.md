...
On *Windows* I recommend using the [MSYS2](https://www.msys2.org/) software for the terminal, and downloading gcc and make within it.
...
This will create a `sudoku_solver` executable file (as well as object files in the `build/` folder) or `sudoku_solver.exe` if on *Windows*. Then you have to run it from terminal with the command (on linux):
...
500630109
```
Where everything after `#` is a comment and the `0` are the representations of the blank spaces in the sudoku puzzle. If you want to add a new Sudoku (make sure it is solvable), there is a blank slot in `puzzle0.sdk`.
